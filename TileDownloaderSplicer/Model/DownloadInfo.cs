...
namespace MapBoard.TileDownloaderSplicer.Model
{
    ...
    {
        public GeoRange<double> MapRange { get; private set; } = new GeoRange<double>();
        ...
        public Dictionary<int, GeoRange<int>> tiles = new Dictionary<int, GeoRange<int>>();

        public event PropertyChangedEventHandler PropertyChanged;

        [JsonIgnore]
        public IReadOnlyDictionary<int, GeoRange<int>> Tiles { get; private set; }

        public void SetRange(GeoRange<double> range)
        {
            MapRange = range;
            int count = 0;
            tiles.Clear();
            for (int level = TileMinLevel; level <= TileMaxLevel; level++)
            {
                ...
                tiles.Add(level, new GeoRange<int>(tile1X, tile2X, tile1Y, tile2Y));

                count += (tile2X - tile1X + 1) * (tile2Y - tile1Y + 1);
            }
            Tiles = new ReadOnlyDictionary<int, GeoRange<int>>(tiles);
            TileCount = count;
        }
        ...
        {
            public TileEnumerator(Dictionary<int, GeoRange<int>> ranges, TileInfo start = null)
            {
                Ranges = new Dictionary<int, GeoRange<int>>(ranges);
                if (start != null)
                {
                    current = start;
            ...
            public Dictionary<int, GeoRange<int>> Ranges { get; }
            object IEnumerator.Current => current;

            private GeoRange<int> currentRange = null;
    ...
    public class TileInfo
    {
        ...
        public GeoRange<double> Extent
        {
            get
            {
                ...
                return new GeoRange<double>(xMin, xMax, yMin, yMax);
            }
        }
    }
}